...
def fritz_request(action, host="fritz.box", port=49000):
    soap_body = '\r\n'.join((
...
def get_ip_address_from_result(result):
    if "urlopen error" in result and ("getaddrinfo failed" in result or "Name or service not known" in result):
        ...
        if "GetExternalIPAddressResponse" in result:
            return result[result.index("<NewExternalIPAddress>") + 22:result.index("</NewExternalIPAddress>")]

def print_fritz_result(result, action, host, port, debug):
    if debug:
...
if __name__ == "__main__":
    ...
    for i, action in enumerate(commands):
        ...
        if(action == "GetExternalIPAddress" and "ForceTermination" in commands[:i]):
            while("Connected" not in fritz_request("GetStatusInfo", host, port)):
                sleep(0.1)

        result = fritz_request(action, host, port)
        print_fritz_result(result, action, host, port, debug)
